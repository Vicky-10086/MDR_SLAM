...
void Viewer::Run()
{
    ...
    while(1)
    {
       ...
       if(menuShowImage)
	   {
	   		// 从FrameDrawer获取带有特征点的当前帧图像
        		cv::Mat im = mpFrameDrawer->DrawFrame();
 			// 【启用】使用OpenCV的imshow在一个新窗口中显示图像
			// "ORB-SLAM2: Current Frame" 是新窗口的标题
			// 我们直接显示原始分辨率的图像 im
       ...
       //向GPU装载图像
		...
		if(menuSaveMap)
        {
            std::cout << "Save Map button clicked. Note: programmatic saving is disabled in this version." << std::endl;       
            menuSaveMap = false;
        }
        if(menuReset)
        {
